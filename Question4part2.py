# -*- coding: utf-8 -*-
"""
Created on Sun Apr 24 02:50:46 2022
...
z=0;    
learning_rate = 0.01
dic=[5] #dictionary object with size 5 which we feed into plot function
...
data_transform = transforms.Compose([
    ...
    ])#initiating normalize functions



#initilize model cnn3 the input and output parameters are as specified
...
lossarray=[[] for i in range(3)]#array to store loss
weigths = [[] for i in range(3)]#array to store weight
valid_accuracy_array = [[] for i in range(3)]#array to store valid
...
#create dataset sampler with 1/10 of size
# Data loader
...
model = cnn3().to(device)#create and send to gpu
total_train=0
correct_train=0
# Loss and optimizer
...
for epoch in range(num_epochs):
    for i, (images, labels) in enumerate(train_loader):  
        ...
        if (i+1) % 10 == 0:#every tenth step save loss and valid acc
            n_samples=0
            n_correct=0
            model.eval()
            with torch.no_grad():
                #evaluator for validacc
